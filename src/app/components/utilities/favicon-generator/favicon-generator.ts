...
/** Blob and DataView both refuse a view over a possibly-shared buffer. */
...
export class FaviconGeneratorComponent implements OnDestroy {
  ...
  downloadZip(): void {
    ...
    this.zipUrl = URL.createObjectURL(
      new Blob([this.buildZip(entries)], { type: 'application/zip' })
    );
  ...
  /**
   * A stored (uncompressed) zip. PNGs are already deflated, so compressing them again
   * buys a rounding error and would cost a deflate implementation.
   */
  buildZip(entries: ZipEntry[]): Bytes {
  ...
  private crc32(bytes: Bytes): number {
